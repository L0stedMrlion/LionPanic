...
if Config.Framework == "esx" then
    ESX.RegisterUsableItem(Config.ItemName, function(source)
        if Config.UseItem then
            ...
            if verify then
                TriggerClientEvent('location:sendCoords', source)
            else
                TriggerClientEvent('location:error', source)
            end
        else
...
RegisterNetEvent('location:syncPosition')
AddEventHandler('location:syncPosition', function(position)
    local src = source
            ...
            if targetPlayer then
                ...
                for _, job in pairs(Config.Jobs) do
                    if targetPlayerJob == job then
                        TriggerClientEvent('location:alarm', playerId, playerName, position)
    ...
    if Config.Framework == "qbcore" then
        ...
        for _, playerId in ipairs(targetPlayers) do
            ...
            if targetPlayer then
                ...
                for _, job in pairs(Config.Jobs) do
                    if targetPlayerJob == job then
                        TriggerClientEvent('location:alarm', targetPlayer.PlayerData.source, playerName, position)
                        SendLog(Webhook.TRIGGERED, Config.Webhooktitle, Config.Translation["panic_triggered"]:format(playerName))
                    end
                end
            end
